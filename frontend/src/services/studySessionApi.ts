...
export async function startStudySession(taskId: number, sessionType = 'new_learning'): Promise<StudySessionItem | null> {
  try {
    return await apiFetch<StudySessionItem>('/api/study-sessions/start', {
      method: 'POST',
...
export async function completeStudySession(
  ...
  try {
    return await apiFetch<StudySessionItem>(`/api/study-sessions/${sessionId}/complete`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function listTaskSessions(taskId: number): Promise<StudySessionItem[]> {
  try {
    return await apiFetch<StudySessionItem[]>(`/api/study-sessions/task/${taskId}`)
  } catch {
    return []
  }
}
...
export async function listActiveStudySessions(taskId?: number): Promise<StudySessionItem[]> {
  ...
  try {
    return await apiFetch<StudySessionItem[]>(`/api/study-sessions/active${qs}`)
  } catch {
    return []
  }
}
