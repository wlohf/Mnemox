...
- 每个 AI Provider 可配置上下文 token 上限和输出 token 上限；长对话会优先保留当前问题与最近上下文
- 常见连接问题会转成可读提示：API Key 错误、模型不存在、额度不足、Base URL 填错、返回非 JSON 或非 OpenAI-compatible 聊天格式

### 7. 联网搜索设置
- 可在 AI 设置页集中管理联网搜索策略：`auto`、供应商 hosted search、Tavily、应用层搜索、DuckDuckGo / Bing 兜底
- Tavily 支持 API Key 加密保存、搜索深度、结果数、chunks、超时和 fallback 开关
- DuckDuckGo / Bing 不需要额外 Key，用作无 Tavily Key 或其他搜索失败时的最终兜底
- 搜索结果会带上来源 provider、评分和发布时间等元信息，方便检查回答引用
...
### 15. 个性化界面与系统设置
- 设置面板：主题、背景图、AI 供应商、联网搜索、token 上限、激励语录、提示词、系统更新
...
5. **联网搜索 → 最新资料**：在 AI 设置页配置 Tavily Key，或不填 Key 直接测试 DuckDuckGo / Bing 兜底搜索，观察回答是否带有可检查的来源。
6. **自定义 Prompt**：在 `/prompts` 调整教练、费曼、苏格拉底等场景提示词，验证 AI 回复风格能否按你的学习偏好变化。
...
v1.3.0 重点将用户笔记接入聊天闭环，并完成当前项目状态、技术边界和发布流程的基线整理：

- **笔记上下文检索**：聊天会按当前用户、关键词和最近更新时间检索相关笔记，优先提供与当前问题匹配的摘录。
- **安全上下文注入**：检索结果经过不可信上下文包装和 1,800 字符预算限制；检索失败会自动跳过，不影响聊天、搜索、记忆和落库流程。
- **聊天证据提示**：流式接口新增笔记上下文 SSE 事件，前端会提示本轮已参考的笔记标题。
- **隔离与回归覆盖**：新增笔记检索排序、跨用户隔离、Prompt Injection 包装、指示器裁剪和聊天事件测试。
- **本地演示数据**：新增 `scripts/seed_showcase_account.py`，可初始化本地演示账号与学习闭环数据；仅限本地 Demo 使用。
- **文档基线**：新增需求、技术和进度文档，统一记录产品范围、工程结构、质量门槛和下一阶段优先级。
- **验证覆盖**：后端 `152 passed, 53 subtests passed`；前端 `19 passed files, 60 passed tests`、build、lint；桌面端 `21 passed`。
- **版本元数据同步**：后端、前端、桌面包、更新清单和发布脚本同步到 `1.3.0`。

上一轮更新日期：2026-06-30

v1.2.0 重点把 Agent 从“可用助手”推进到“目标驱动学习 cockpit”，并补齐 Coach、记忆、笔记证据和搜索质量闭环：

- **目标驱动 Agent cockpit**：Agent 页新增当前目标、今日最小行动、目标风险信号、相关笔记 / 错题 / 复习证据和行动草案入口。
- **双层记忆系统**：新增短记忆、长期记忆候选、Core Profile、记忆确认 / 锁定 / 忽略和可手动触发的学习流程。
- **自主 Coach Kernel**：新增 Coach 事件、nudges、偏好、免打扰、冷却、每日上限、桌面通知、技能注册和反馈学习统计。
- **笔记证据工作流**：笔记页新增编辑 / 预览 / 分屏模式、goal/task/material/review 关联、选中文本生成复习提示、生成任务和安全问 Agent 预览。
- **搜索质量与缓存**：新增搜索结果去重、质量增强、缓存表和 RAG 状态提示优化，降低重复搜索和引用不稳定。
- **桌面背景持久化修复**：自定义背景图走后端存储，番茄背景同步到桌面偏好，避免重启后回退；本地验证产物加入忽略规则。
- **验证覆盖**：新增 Agent 评测、Agent 记忆、Coach Kernel、目标上下文、学习事件、笔记检索、搜索缓存和前端服务测试。
- **版本元数据同步**：应用版本、前端包版本、桌面包版本、更新清单和发布脚本默认版本同步到 `1.2.0`。

上一轮更新日期：2026-06-22

v1.1.0 重点补齐了更适合公开试用的联网搜索和 token 预算配置：

- **Tavily 高质量搜索**：AI 设置页新增持久化搜索设置，可配置 Tavily API Key、搜索深度、结果数、chunks、超时和 fallback 开关。
- **DuckDuckGo / Bing 最终兜底**：没有配置 Tavily Key，或 Tavily、Grok/专用搜索、Responses hosted search、工具搜索失败时，会降级到无需 Key 的 DuckDuckGo / Bing 搜索。
...
3. 如果要体验联网搜索，打开 AI 设置里的“联网搜索”，可填写 Tavily Key；不填 Key 时也可以测试 DuckDuckGo / Bing 兜底。
4. 上传一份小型 Markdown / TXT / PDF 学习资料。
5. 创建一个项目，把资料关联进去。
6. 在 AI 对话中询问资料内容，确认资料引用和回答正常。
7. 记录几条错题、写一条笔记、完成一次复习。
8. 使用番茄钟完成几次学习后，到用户画像页刷新画像。
...
**Q: 没有 Tavily Key 还能联网搜索吗？**

A: 可以。Tavily 是更高质量的可选搜索路径；没有 Tavily Key 时，应用层搜索会使用 DuckDuckGo / Bing 作为最终兜底。搜索质量会受搜索引擎页面结构和网络环境影响。

**Q: AI token 上限应该怎么设置？**

A: 输出 token 上限控制回答最长长度；上下文 token 上限控制发送给模型的聊天历史预算。一般可以先保持默认，需要长文回答时调高输出上限，长对话容易超上下文时再调高上下文上限。
