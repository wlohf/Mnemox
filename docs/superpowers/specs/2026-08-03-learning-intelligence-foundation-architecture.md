...
| 混合检索 | 资料 RAG 使用 Chroma + 关键词降级；聊天笔记已完成首条 `ContextStore` 接口迁移，当前实现为关键词 SQL 基线 | Qdrant 是 `ContextStore` 的正式 Spike 候选 | Windows 桌面数据目录与升级、无 embedding Key 降级、用户过滤、更新/删除/重建、离线质量集、延迟与成本均通过 |
| 概念关系查询 | SQL 三表和局部邻域 | Neo4j 是 `GraphStore` Spike 候选 | 真实多跳/编辑场景明显优于 SQL；桌面部署、备份恢复、权限、关系溯源和迁移可接受 |
| 时态记忆 | SQL `UserMemory` 当前投影、`MemoryDeclaration` 审计历史、会话摘要与事件 | Graphiti 是 `TemporalMemoryGraph` Spike 候选；桌面端同时评估嵌入式图后端与服务端后端 | 有效时间/失效关系正确、只摄入筛选 episode、可按用户删除并从 SQL 重建、LLM/图后端不可用时安全降级 |
| 学习调度 | `py-fsrs` 已接入并完成版本化迁移、存量数据保留回归和一次性 PostgreSQL 16 演练 | **保留并深化** | 正式发布窗口继续验证时区、到期语义和真实历史校准 |
...
截至 2026-08-19，步骤 1/2/6 已形成可验证的主体切片，步骤 3 的聊天笔记路径已完成接口收敛；可重建检索投影、版本更新/删除残留、离线质量集，以及步骤 4/5 的候选 Spike 仍未完成。这里的顺序继续约束后续工作，不因首条迁移完成而放宽验收门槛。
...
- 当前 Chroma 与关键词路径仍是运行时基线；聊天笔记已完成首条 `ContextStore` 接口迁移，但候选检索、独立投影和完整更新/删除闭环通过验收前，不替换资料 RAG 或扩大候选组件所有权。
