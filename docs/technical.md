...
> 基线日期：2026-08-04
>
> 当前版本：v1.3.0（Phase 1 projection-outbox 收口）
> 代码范围：当前 `feature/phase2-agent-upgrade` 工作区；学习者模型首个切片已验证，outbox/API 主体和数据库演练已完成，最新正确性加固待最终回归；未提交的 AgentKernel 仍仅作为进行中原型记录
...
当前后端包含 30 个路由模块，按 `/api/*` 前缀组织。主要领域如下：
...
- `app/services/`：当前有 45 个服务模块，处理用户画像、事件、学习者模型、投影、记忆、搜索缓存、笔记上下文、Agent 学习、Coach 策略和检索等。
...
- 当前稳定主路径仍是规则简报与一次性 Planner；工作区内的 AgentKernel 原型支持多步只读工具调用和执行日志，但尚未接入前端、SSE 或草案执行闭环，不能视为已替代 Planner。
