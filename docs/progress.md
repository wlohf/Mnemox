...
>
> 当前版本：v1.3.0
> 当前阶段：Phase 1 学习者模型切片已收口；生产化与真实数据校准进行中；Phase 2 等待 Phase 1 验收

本文件记录可执行的项目状态。需求范围见 [需求文档](requirements.md)，工程实现见 [技术文档](technical.md)，按周变化记录见 `docs/updates/`。

## 1. 阶段判断

Mnemox 已完成核心学习工具、个性化学习闭环、Agent/Coach 和 Windows 桌面交付的第一轮实现。当前处于 **v1.3.0 的 Beta/Phase 1 生产化阶段**：默认 SQLite 升级和一次性 PostgreSQL 演练已完成；学习者模型、同事务 projection outbox、分页重放、API、前端证据下钻、校准基线和 PostgreSQL 常驻 worker 均已验证。下一步是积累真实 holdout 数据、补失败队列监控/告警，并按发布窗口执行正式生产升级；Phase 2 不提前展开。
...
| 当前工作分支 | `feature/phase2-agent-upgrade`；工作区有未提交的学习者模型/outbox/API 与 AgentKernel 路由、实现和测试，不能视为已发布能力。 |
...
| Phase 1 | 四层学习智能底座：数据契约、事件投影、ContextStore/Qdrant Spike、概念图/Neo4j Spike、时态记忆/Graphiti Spike、学习者模型、Obsidian 与联想 | 🔶 MVP 部分完成；学习者模型/outbox/API/前端证据下钻、离线校准基线、数据库演练和 PostgreSQL 常驻 worker 已验证；失败监控、统一检索迁移、候选 Spike、同步冲突/删除、Coach 联想反馈和真实数据质量评测待补 |
...
2. **生产化 outbox**：补失败队列监控、告警和跨实例聚合指标；保留 PostgreSQL 常驻 worker 与请求内事件级消费作为低延迟路径。
3. **Phase 1 其余底座**：继续记忆声明、真实 `ContextStore` 业务流、Obsidian 删除/冲突和 Coach 联想反馈。
4. **兼容退出**：兼容周期和数据校准验收完成后，再规划移除 `Concept.mastery`。
5. **候选 Spike**：按路线图依次评估 Qdrant、Neo4j、Graphiti；每个候选都要有质量、桌面分发、迁移、删除、隔离和降级证据，失败则继续使用现有基线。
6. **Phase 2（等待 Phase 1 验收）**：再比较 AgentKernel 与 LangGraph，随后才进入 SSE/草案闭环、后台调度、自学习归因和知识写回。
