...
def _get_effective_values(row: AIProviderSetting) -> tuple[str, str, str]:
    ...
    api_key = row.api_key or (getattr(settings, env_attrs[0], "") if env_attrs[0] else "")
...
async def update_routing(
    ...
    row = result.scalar_one_or_none()
    if not row:
        row = AIRoutingSetting(user_id=current_user.id, scenario=scenario, provider_name=provider_name)
        db.add(row)
    else:
