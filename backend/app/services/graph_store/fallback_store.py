...
class FallbackGraphStore:
        ...
        try:
            ...
            try:
                ...
                self.last_diagnostics = {
                    ...
                    "fallback_latency_ms": round((time.perf_counter() - fallback_started) * 1000.0, 3),
                }
                raise
            self.last_diagnostics = {
                "backend": self.backend,
    ...
    async def expand_claims(self, *, user_id: int, claim_ids: Sequence[int], patterns: Sequence[str], depth: int = 1, limit: int = 50) -> list[GraphHit]:
        return await self._read_with_fallback(operation="expand_claims", primary_call=lambda: self.primary.expand_claims(user_id=user_id, claim_ids=claim_ids, patterns=patterns, depth=depth, limit=limit), fallback_call=lambda: self.fallback.expand_claims(user_id=user_id, claim_ids=claim_ids, patterns=patterns, depth=depth, limit=limit))
    ...
    async def expand_concepts(self, *, user_id: int, concept_ids: Sequence[int], patterns: Sequence[str], depth: int = 1, limit: int = 50) -> list[GraphHit]:
        return await self._read_with_fallback(operation="expand_concepts", primary_call=lambda: self.primary.expand_concepts(user_id=user_id, concept_ids=concept_ids, patterns=patterns, depth=depth, limit=limit), fallback_call=lambda: self.fallback.expand_concepts(user_id=user_id, concept_ids=concept_ids, patterns=patterns, depth=depth, limit=limit))

    async def source_claims(self, *, user_id: int, source_id: int, limit: int = 50) -> list[GraphHit]:
        return await self._read_with_fallback(operation="source_claims", primary_call=lambda: self.primary.source_claims(user_id=user_id, source_id=source_id, limit=limit), fallback_call=lambda: self.fallback.source_claims(user_id=user_id, source_id=source_id, limit=limit))

    async def find_concept_paths(self, *, user_id: int, start_concept_ids: Sequence[int], target_concept_ids: Sequence[int], relation_types: Sequence[str], direction: TraversalDirection = "outgoing", max_depth: int = 4, limit: int = 10) -> list[GraphPath]:
        return await self._read_with_fallback(operation="find_concept_paths", primary_call=lambda: self.primary.find_concept_paths(user_id=user_id, start_concept_ids=start_concept_ids, target_concept_ids=target_concept_ids, relation_types=relation_types, direction=direction, max_depth=max_depth, limit=limit), fallback_call=lambda: self.fallback.find_concept_paths(user_id=user_id, start_concept_ids=start_concept_ids, target_concept_ids=target_concept_ids, relation_types=relation_types, direction=direction, max_depth=max_depth, limit=limit))
