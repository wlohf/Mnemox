...
class RestartAfterInterruptionSkill(CoachSkill):
    ...
    tone_rules = ["不责备", "缩短下一轮时长", "强调恢复节奏", "引用用户笔记时只用原文并注明出处"]
    ...
    async def generate(self, ctx: CoachSkillContext) -> CoachSkillResult:
        ...
        signals = [f"stop_reason={payload.get('stop_reason') or ctx.event.get('event_type')}"]
        body, used_quote = render_note_quote(ctx, body, signals)
        explainability = explain_with_context(ctx, "番茄钟被中断，短时重启比继续加压更稳。", signals)
        if used_quote:
