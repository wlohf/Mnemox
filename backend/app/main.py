        ...
        try:
            ...
            if status.get("total_chunks", 0) == 0:
                async with async_session_maker() as session:
                    ...
                    if total > 0:
                        logger.info("首次运行，正在索引 %d 份已有资料...", total)
                        ...
                        logger.info("自动索引完成，共索引 %d 份资料", total)
        except Exception as e:
...
# 引入路由
from app.routers import materials, pomodoro, rag, plans, ai_settings, chat, conversations, chat_projects, wrong_questions, review, goals, study_sessions, memory, notes, learning, images, obsidian_import, auth, motivation, profile, prompt_templates
